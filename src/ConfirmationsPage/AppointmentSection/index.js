...
import {
  getAllHistories,
  getHistoriesForAdmission,
  getAdmissionById,
} from "../../services/admissions.services";
...
export const AppointmentSection = ({ id }) => {
  ...
  useEffect(() => {
    const getHistorial = async () => {
      ...
      const evolutionList = list.filter(
        (item) => item.plantilla === "649c76f33143bd2e71cedad8"
      );

      //PARA OBTENER LA LISTA DE FORMULARIOS DE VALORACIONES
      const valorationList = list.filter(
        (item) => item.plantilla === "649c76fb3143bd2e71cedada"
      );
      ...
      if (admision) {
        setSessions(
          admision[0].body
            ...
            .campos.find((campo) => campo.titulo === "Sesiones Ordenadas").valor
        );
      }
      // console.log(list);
  ...
  return (
    <>
          ...
          {evaluaciones && (
            <>
              <div className="card mb-2">
                <div className="card-body citas-admision-body">
                  <div className="row">
                    <p className="my-auto col-6">
                      <strong className="me-2">Valoración:</strong>
                      {valoracion.length} / 1
                    </p>
      ...
      {/* {evaluaciones && (
        <>
          <p>
            sesiones ordenadas: <b>{sessions}</b>
          </p>
          <p>
            cantidad de citas: <b>{evaluaciones.length}</b>{" "}
          </p>
        </>
      )} */}
      {/* {admission && (
